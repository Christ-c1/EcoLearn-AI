...
# JWT Configuration - Use environment variables with fallback
JWT_SECRET_KEY = os.environ.get("JWT_SECRET_KEY", "ecolearn-secret-key-change-in-production-2024")
...
async def get_jwks() -> Dict[str, Any]:
    ...
    oidc_issuer_url = getattr(settings, 'oidc_issuer_url', None)
    if not oidc_issuer_url:
        raise Exception("OIDC issuer URL not configured")
    
    jwks_url = f"{oidc_issuer_url}/.well-known/jwks.json"
    try:
...
def create_access_token(claims: Dict[str, Any], expires_minutes: Optional[int] = None) -> str:
    ...
    expiry_minutes = expires_minutes if expires_minutes is not None else JWT_EXPIRE_MINUTES
...
async def validate_id_token(id_token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            oidc_issuer_url = getattr(settings, 'oidc_issuer_url', 'unknown')
            logger.error(
                f"ID token validation failed: Failed to fetch JWKS from issuer {oidc_issuer_url}: {e}"
            )
        ...
        if not key:
            oidc_issuer_url = getattr(settings, 'oidc_issuer_url', 'unknown')
            logger.error(
                f"ID token validation failed: No key found for kid: {kid} in JWKS from {oidc_issuer_url}"
            )
...
def build_authorization_url(
    ...
    oidc_issuer_url = getattr(settings, 'oidc_issuer_url', None)
    oidc_client_id = getattr(settings, 'oidc_client_id', None)
    oidc_scope = getattr(settings, 'oidc_scope', 'openid profile email')
    ...
    auth_url = f"{oidc_issuer_url}/authorize?" + urllib.parse.urlencode(params)
    return auth_url
